...
export default function CommentsNotification() {
  const [onPushNotification, setOnPushNotification] = useState(true);
  const toggleSwitchOnPush = () => {
    if (onPushNotification) {
      console.log('Set off push notification with comments')
    } else {
      console.log('Set on push notification with comments')
    }
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Text style={styles.normalText}>Đây là thông báo khi có người bình luận về bài viết và
        phản hồi bình luận của bạn. Sau đây là ví dụ.</Text>
      <View style={[styles.rowElement, styles.status]}>
        ...
        ><Text style={{fontWeight: '700'}}>Josephine Williams</Text> đã trả lời một bình luận có gắn thẻ bạn.</Text>
      </View>
      <View>
